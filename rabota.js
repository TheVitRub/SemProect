...
async function  Pers_icon(data, id){
    ...
    let personal_name = '<td  id="tr_2' + id +'"><h2>Имя персонажа: </br></h2><h3>'+data[0].name+'</h3></td>'
    let origin_name = '<td id="tr_3' + id +'" ><h2>Место рождения: </br></h2><h3>'+data[0].origin.name+'</h3></td>'
    let location_name = '<td id="tr_4' + id +'"><h2>Место проживания: </br></h2><h3>'+data[0].location.name+'</h3></td>'
    let first_tr = '<tr>' + photo + personal_name + origin_name + location_name + '</tr>'

    let per_status = '<td id="td_5' + id +'" style="width: 75px;height: 150px;"><h2>Нынешний статус:</h2></br><h3>'+data[0].status+'</td>'
    let planet_status_loc = '<td id="td_6' + id +'" style="width: 75px;height: 150px;" rowspan="3"><h2>Статус местонахождения:</br></h2><h3>'+loc_planet[0].status+'</h3></br><h2>Тип планеты местонахождения:</br></h2><h3>'+loc_planet[0].species+'</h3></td>'
    let planet_status_ori = '<td id="td_7' + id +'" style="width: 75px;height: 150px;" rowspan="3"><h2>Статус планеты рождения:</br></h2><h3>'+or_planet[0].status+'</h3></br><h2>Тип планеты рождения:</h2><h3>'+or_planet[0].species+'</h3></td>'
    let second_tr = '<tr>' +per_status + planet_status_loc + planet_status_ori + '</tr>'

    let per_species = '<td  id="tr_8' + id +'"><h2>Раса:</br></h2><h3>'+data[0].species+'</h3></h2></td>'
    let therd_tr = '<tr>' +per_species + '</tr>'

    let per_gender = ' <td  id="tr_9' + id +'"><h2>Гендер:</br></h2><h3>'+data[0].gender+'</h3></h2></td>'
